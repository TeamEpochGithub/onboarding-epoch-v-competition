...
def run_train_cfg(cfg: TrainConfig) -> None:
    """Train a model pipeline with a train-test split."""
    print_section_separator("Onboarding Epoch V Competition - Training")
    import coloredlogs
    ...
    if not x_cache_exists:
        X = setup_train_x_data(cfg.train_path, cfg.image_metadata_path, cfg.pokemon_metadata_path)

    if not y_cache_exists:
        y = setup_train_y_data(cfg.image_metadata_path, cfg.pokemon_metadata_path)
    ...
    if cfg.test_size == 0:
        ...
        train_indices: list[int] = list(range(len(X))) if X is not None else []
